...
Posts.deny({
  update: function(userId, post, fieldNames) {
    ...
    return (_.without(fieldNames, 'categories', 'title','summary','introduction','list').length > 0);
  }
});
...
Meteor.methods({
  postInsert: function(postAttributes) {
    ...
    var postWithSameCategory = Posts.findOne({category: postAttributes.category});
    if (postWithSameCategory) {
      return {
        postExists: true,
        _id: postWithSameCategory._id
      }
    }
